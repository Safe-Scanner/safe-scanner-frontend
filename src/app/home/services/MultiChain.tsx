...
function MultiChain() {
	...
	return (
		<FeatureItem
			title="Multi Chain"
			description="Supported blockchains & tech behind"
		>
			<Stack direction="row" gap={1} flexWrap="wrap">
				{networks.map(({ icon, label, isNew }, index) => (
					<Box key={index} sx={{ position: "relative", zIndex: 0 }}>
						<Chip
							icon={<Box component="img" alt="" width={20} src={icon} />}
							label={label}
							color="white"
						/>
						{isNew && (
			...
			<Box>
				<Button
					startIcon={
						<Image src="/images/file-code.svg" alt="" width={24} height={24} />
					}
					size="large"
					disabled={true}
				>
