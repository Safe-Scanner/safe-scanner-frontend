...
function Transactions() {
	...
	const [moduleTransactions, setModuleTransactions] = useState([] as any);
	...
	useEffect(() => {
		...
		if (transaction != null) {
			...
			temp.forEach((el: any) => {
				...
				if (el?.txType === "MODULE_TRANSACTION") {
					setModuleTransactions((prev: any) => [
						...prev,
						{
							...
							moduleTransactionHash: el?.moduleTransactionId,
	...
	return (
		<Grid container sx={{ marginTop: 2 }} spacing={0.5}>
			...
			{moduleTransactions.length > 0 &&
				moduleTransactions.map((el: any, index: any) => (
					<Grid key={index} item xs={12} md={12} lg={12}>
						<ModuleTransaction
							value={el?.value}
							safeTxHash={el?.safeTxHash}
							...
							txHash={el?.moduleTransactionHash}
							network={network}
							method={el.method}
						/>
					</Grid>
